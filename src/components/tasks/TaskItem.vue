<template>
    <div
        ...
        <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span v-if="local.priority" class="badge badge-sm shrink-0"
                        :class="priorityClass">{{ priorityLabel }}</span>
                </div>
                <p class="text-sm text-base-content/60 mt-1 line-clamp-2">{{ descriptionPreview }}</p>
            </div>
            <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                <button class="btn btn-ghost btn-xs" title="Sửa" @click="$emit('edit', local)"><Pencil class="w-3 h-3"/></button>
                <button class="btn btn-ghost btn-xs text-error" title="Xoá" @click="$emit('delete', local)"><Trash2 class="w-3 h-3"/></button>
            </div>
        </div>
        <div class="mt-3 flex items-center justify-between">
            <button class="btn btn-ghost btn-xs gap-1" @click="$emit('view', local)"><FileText class="w-3 h-3"/> Xem</button>
        ...
        <div v-if="local.status != null" class="mt-2 flex items-center gap-2">
            <span class="text-xs text-base-content/60">Trạng thái:</span>
            <select class="select select-bordered select-xs" v-model="local.status"
